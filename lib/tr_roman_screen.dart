...
class _TrRomanScreenState extends State<TrRomanScreen> {
  ...
  Future<void> _loadRomansPage() async {
    ...
    try {
      ...
      final String query =
          'SELECT id, KitapAdi, KitapYazar, KitapNo FROM dbo.TrRoman $whereClause ORDER BY id OFFSET $_offset ROWS FETCH NEXT $_pageSize ROWS ONLY';
  ...
  Future<void> _deleteRoman(dynamic idDynamic) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Bu Türkçe roman kaydını kalıcı olarak silmek istediğinizden emin misiniz?',
        ),
        actions: [
          TextButton(
    ...
    try {
      final String query = 'DELETE FROM TrRoman WHERE id = $id';
  ...
  void _navigateToAddEdit({Map<String, dynamic>? roman}) async {
    final shouldRefresh = await Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            AddEditRomanScreen(sqlService: widget.sqlService, roman: roman),
      ),
    );
